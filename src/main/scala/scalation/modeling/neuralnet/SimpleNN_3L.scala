...
@main def simpleNN_3L2 (): Unit =
    ...
    for epoch <- 1 to 10 do
        ...
        for i <- x.indices do
            ...
            z = f_sigmoid.f_ (v)                                // hidden activation vector using f0 = sigmoid
            u = b.ᵀ * z + β                                     // output pre-activation vector
            ŷ = f_sigmoid.f_ (u)                                // output prediction vector using f1 = sigmoid
        ...
        val r2 = VectorD.one (sse.dim) - sse / sst              // R^2
