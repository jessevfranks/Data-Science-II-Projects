...
import scalation.modeling.ActivationFun.{f_id, f_sigmoid}
import scalation.modeling.neuralnet.{NeuralNet_2L, NeuralNet_3L}
import scalation.modeling.{AFF, Regression, qk}
...
class Project2Utils(x: MatrixD, y: VectorD, fname: Array[String]) {
  ...
  def runNeuralNet2L(eta: Double = 0.1, bSize: Int = 20, f: AFF = f_sigmoid): Unit = {
    setEta(eta)
    setBatchSize(bSize)
    val mod = NeuralNet_2L.perceptron(x, y, fname, f = f)
  ...
  def runNeuralNet3L(eta: Double = 0.1, bSize: Int = 20, f: AFF = f_sigmoid, f1: AFF = f_id, nz: Int = -1): Unit = {
    setEta(eta)
    setBatchSize(bSize)
    
    val mod = NeuralNet_3L.rescale(x, yMat, fname, nz, f = f, f1 = f1)
