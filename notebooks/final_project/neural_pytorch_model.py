...
def tune_hyperparameters(X_train, y_train, numeric_features, categorical_features,
    ...
    print("CV folds: 3, scoring: F1 on minority class\n")
    ...
    for lr, dropout in product(LEARNING_RATES, DROPOUTS):
        ...
        print(
            f"  lr={lr:<8}  dropout={dropout:<5}  "
            f"F1 = {mean_f1:.4f} +/- {std_f1:.4f}  ({elapsed:.1f}s)"
        )
...
def run_experiments_for_dataset(dataset_name, X, y, numeric_features,
    ...
    for arch, label in ARCHITECTURES:
        result = run_single_training(
            ...
            categorical_features,
            hidden_sizes=arch,
            label=f"PyTorch NN Sigmoid — {label} (untuned)",
            dataset_name=dataset_name,
            cost_matrix=cost_matrix,
            lr=0.001,
            dropout=0.2,
    ...
    print(
        f"\nBest architecture on {dataset_name}: "
        f"{best_arch_label} (F1 = {phase1_df.loc[best_idx, 'F1']:.4f})"
    )
    ...
    tuned_result = run_single_training(
        ...
        categorical_features,
        hidden_sizes=best_arch,
        label=f"PyTorch NN Sigmoid — {clean_arch_label} (TUNED lr={best_lr}, dropout={best_dropout})",
        dataset_name=dataset_name,
        cost_matrix=cost_matrix,
        lr=best_lr,
...
def threshold_tuning_analysis(models_dict, X, y, numeric_features,
    ...
    print(
        f"Theoretical optimal threshold for this cost ratio: "
    ...
    for model_name, pipeline in models_dict.items():
        ...
        df, best_t, best_cost = find_best_threshold(
            pipeline,
            X_test,
            y_test,
            cost_matrix,
        ...
        print(
            f"  -> Best threshold ({best_t}) cost: {int(best_cost)} "
            f"(reduction: {int(default_cost - best_cost)})"
        )
...
def main():
    ...
    ger_models_for_threshold = {
        "PyTorch NN Sigmoid — 2 Hidden (tuned)": Pipeline(steps=[
    ...
    output_path = "/Users/aidyn/Desktop/ScalaTion-Projects/data/finalProject/pytorch_model_results_sigmoid.csv"
